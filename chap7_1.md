...
Let $\mathcal B$ be a belief space equipped with an energy quasi-metric $d$ and a debt function $D=\delta\psi$. For every pair with $d(b_i,b_j)<+\infty$, define
...
**Proposition 7.4 (Basic properties).**
 
For all $b,b_i,b_j\in\mathcal B$ (with $d(b_i,b_j)<+\infty$):
...
Thus the diagonal of $Q$ is zero, and all finite transitions lie in the closed right half-plane $\operatorname{Re}Q\ge 0$.
...
**Definition 7.6 (Complex transition magnitude).**
...
$$
\boxed{
\rho(b_i,b_j)=\sqrt{d(b_i,b_j)^2+D(b_i,b_j)^2}.
}
$$
...
For every finite transition, $\rho(b_i,b_j)\geq0$ and $\rho(b,b)=0$. Moreover,
 
$$
\rho(b_i,b_j)=0
\iff
d(b_i,b_j)=0 \text{ and } D(b_i,b_j)=0.
$$
 
### Proof
 
Both $d^2$ and $D^2$ are nonnegative, so their sum has a nonnegative square root, which vanishes exactly when both terms vanish. On the diagonal, $d(b,b)=D(b,b)=0$. $\square$
...
**Theorem 7.8 (Triangle inequality for the modulus).**
 
If $d$ satisfies the quasi-metric triangle inequality, then for all $b_i,b_j,b_k$ with finite costs,
 
$$
\boxed{
\rho(b_i,b_k)\leq\rho(b_i,b_j)+\rho(b_j,b_k).
}
$$
...
Since $0\le d(b_i,b_k)$, squaring preserves the inequality, so
 
$$
\rho(b_i,b_k)
\leq
\sqrt{\bigl(d(b_i,b_j)+d(b_j,b_k)\bigr)^2+\bigl(D(b_i,b_j)+D(b_j,b_k)\bigr)^2}.
$$
...
$$
\rho(b_i,b_k)\leq\rho(b_i,b_j)+\rho(b_j,b_k). \qquad\square
$$
...
If, in addition, $\rho(b_i,b_j)=0\Rightarrow b_i=b_j$, then $\rho$ is a metric.
 
### Proof
 
If $d(b_i,b_j)=d(b_j,b_i)$, then since $D(b_j,b_i)^2=D(b_i,b_j)^2$,
...
**Proposition 7.16 (Phase of the reverse transition).**
...
If $d$ is symmetric, $Q(b_j,b_i)=\overline{Q(b_i,b_j)}$, and conjugation negates the argument. $\square$
...
Geometrically, each transition is a point
 
$$
(d,D)\in[0,+\infty)\times\mathbb R.
$$
 
The distance from the origin is $\rho$; the angle from the positive cost axis is $\theta$. The complex quasi-metric thus gives a **cost-debt geometry**: the magnitude tells us how large the combined transition is, and the phase tells us how that magnitude is distributed between cost and debt.
...
## 7.15 The role of the potential and the basepoint
...
**Proposition 7.17 (Basepoint form).**
 
Let $s_0\in\mathcal B$ be a basepoint and define $\psi_{s_0}(b):=D(s_0,b)$. Then
...
By additivity and antisymmetry of $D$,
 
$$
D(b_i,b_j)=D(b_i,s_0)+D(s_0,b_j)=\psi_{s_0}(b_j)-\psi_{s_0}(b_i). \qquad\square
$$
 
**Corollary 7.18 (Basepoint independence).**
 
$Q$ does not depend on the choice of basepoint $s_0$.
 
### Proof
 
Two basepoints give potentials differing by a constant, and constants cancel in differences. $\square$
 
**Remark 7.19.** The basepoint is a computational device: it lets us compute $Q$ without reference to paths. The object $Q$, and therefore $\rho$ and $\theta$, is basepoint-independent.
 
**Remark 7.20 (On gauge transformations).** Replacing $\psi$ by $\psi+c$ with $c$ constant leaves $D$ unchanged, so $Q$, $\rho$ and $\theta$ are invariant. If instead $\psi$ is replaced by $\psi+\chi$ with $\chi$ *non-constant*, one obtains a **different** debt function $D'=D+\delta\chi$, and
 
$$
Q'(b_i,b_j)=Q(b_i,b_j)+i\bigl(\chi(b_j)-\chi(b_i)\bigr).
$$
 
This is a translation of $Q$ in the imaginary direction that depends on the pair $(b_i,b_j)$. The real part $d$ is unchanged, but the modulus and phase are **not** invariant under such a transformation. Only $d$ and the constant-shift class of $\psi$ are canonical; the gauge structure is developed in a later chapter.
 
---
 
## 7.16 Debt along a path
...
## 7.17 Algebra of complex quasi-metrics
 
We now consider algebraic operations.
 
**Definition 7.21 (Addition).** For $Q_1=d_1+iD_1$ and $Q_2=d_2+iD_2$,
 
$$
Q_1+Q_2:=(d_1+d_2)+i(D_1+D_2).
$$
 
**Proposition 7.22.** The sum of two complex quasi-metrics is a complex quasi-metric.
 
*Proof.* The real part $d_1+d_2$ is a sum of quasi-metrics, hence a quasi-metric. The imaginary part $D_1+D_2$ is additive and antisymmetric, and $D_1+D_2=\delta(\psi_1+\psi_2)$ is exact. $\square$
 
**Definition 7.23 (Scalar multiplication).** For $\alpha\geq0$,
 
$$
\alpha Q:=(\alpha d)+i(\alpha D).
$$
 
**Proposition 7.24.** If $\alpha\geq0$, then $\alpha Q$ is a complex quasi-metric.
 
*Proof.* $\alpha d$ is a quasi-metric for $\alpha\ge0$, and $\alpha D=\delta(\alpha\psi)$ is exact. $\square$
 
**Remark 7.25.** For $\alpha<0$ the real part $\alpha d$ becomes negative, so scalar multiplication is only defined for $\alpha\geq0$.
 
**Definition 7.26 (Conjugation).** $\overline{Q}:=d-iD$.
 
**Proposition 7.27.** The conjugate satisfies
 
$$
\operatorname{Re}\overline{Q}=\operatorname{Re}Q,
\quad
\operatorname{Im}\overline{Q}=-\operatorname{Im}Q,
\quad
|\overline{Q}|=|Q|,
\quad
...
**Remark 7.28.** Conjugation reverses the sign of the debt and leaves the cost unchanged. It is the complex quasi-metric of the pair $(d,-D)$, with potential $-\psi$. As Section 7.12 shows, it coincides with *reversing the transition* only when $d$ is symmetric.
 
**Definition 7.29 (Product).**
 
$$
Q_1Q_2:=(d_1d_2-D_1D_2)+i(d_1D_2+d_2D_1).
$$
 
**Proposition 7.30.** The product of two complex quasi-metrics is in general **not** a complex quasi-metric.
 
*Proof.* The real part $d_1d_2-D_1D_2$ may be negative, violating nonnegativity. $\square$
 
**Remark 7.31.** The complex quasi-metric is closed under addition and nonnegative scaling, but not under multiplication. The theory is therefore developed in the *additive* regime.
 
---
 
## 7.18 Complex quasi-metrics as 1-cochains
 
A **complex 1-cochain** on $\mathcal B$ is a function $Q:\mathcal B\times\mathcal B\to\mathbb C$.
 
**Proposition 7.32.** The complex quasi-metric is a complex 1-cochain satisfying
 
1. $Q(b,b)=0$;
2. $\operatorname{Re}Q\geq0$;
3. $\operatorname{Im}Q$ is a real 1-coboundary, $\operatorname{Im}Q=\delta\psi$.
*Proof.* Immediate from the definitions. $\square$
 
**Remark 7.33.** Condition 3 distinguishes $Q$ from a general complex 1-cochain. It says that the imaginary part is *integrable*: it is the coboundary of a 0-cochain.
 
---
 
## 7.19 A two-state example
...
|Q(b_1,b_2)|=\sqrt{13},
\qquad
\theta(b_1,b_2)=\operatorname{atan2}(2,3).
$$
...
Q(b_1,b_2)=3+2i,
\qquad
Q(b_2,b_1)=3-2i=\overline{Q(b_1,b_2)}.
$$
 
Moreover
 
$$
|Q(b_1,b_2)|=|Q(b_2,b_1)|=\sqrt{13},
\qquad
\theta(b_2,b_1)=-\theta(b_1,b_2).
$$
...
## 7.21 Structure versus representation
...
## 7.22 Summary of the main relations
...
\boxed{Q=d+iD},
\qquad
\boxed{|Q|=\sqrt{d^2+D^2}},
\qquad
\boxed{\arg Q=\operatorname{atan2}(D,d)},
$$
...
## 7.23 What we have established
 
- **Definition.** $Q(b_i,b_j)=d(b_i,b_j)+iD(b_i,b_j)$, with $\operatorname{Re}Q=d$ and $\operatorname{Im}Q=D=\delta\psi$.
- **Basic properties.** $Q(b,b)=0$, $\operatorname{Re}Q\ge0$, and $\operatorname{Im}Q$ is antisymmetric and exact.
...
\qquad
D=\begin{pmatrix}0&1&3\\-1&0&2\\-3&-2&0\end{pmatrix}.
$$
 
Compute $Q$, $|Q|$ and $\theta$ for all pairs, and verify the triangle inequality for $|Q|$. Is $D$ exact, i.e. additive?
...
**Exercise 7.14.** Give a counterexample showing that the product $Q_1Q_2$ of two complex quasi-metrics need not be a complex quasi-metric.
 
---
 
## 7.25 Final perspective
